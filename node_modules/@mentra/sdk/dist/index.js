...
var init_streams = __esm(() => {
  ((StreamType2) => {
    ...
    StreamType2["RTMP_STREAM_STATUS"] = "rtmp_stream_status";
  ...
  STREAM_CATEGORIES = {
    ...
    ["rtmp_stream_status" /* RTMP_STREAM_STATUS */]: "hardware" /* HARDWARE */,
...
var init_message_types = __esm(() => {
  ...
  ((GlassesToCloudMessageType2) => {
    ...
    GlassesToCloudMessageType2[GlassesToCloudMessageType2["RTMP_STREAM_STATUS"] = "rtmp_stream_status" /* RTMP_STREAM_STATUS */] = "RTMP_STREAM_STATUS";
  ...
  ((CloudToGlassesMessageType2) => {
    ...
    CloudToGlassesMessageType2["START_RTMP_STREAM"] = "start_rtmp_stream";
    CloudToGlassesMessageType2["STOP_RTMP_STREAM"] = "stop_rtmp_stream";
    CloudToGlassesMessageType2["KEEP_RTMP_STREAM_ALIVE"] = "keep_rtmp_stream_alive";
  ...
  ((AppToCloudMessageType2) => {
    ...
    AppToCloudMessageType2["RTMP_STREAM_REQUEST"] = "rtmp_stream_request";
    AppToCloudMessageType2["RTMP_STREAM_STOP"] = "rtmp_stream_stop";
  ...
  ((CloudToAppMessageType2) => {
    ...
    CloudToAppMessageType2["RTMP_STREAM_STATUS"] = "rtmp_stream_status";
...
function isStartRtmpStream(message) {
  return message.type === "start_rtmp_stream" /* START_RTMP_STREAM */;
}
function isStopRtmpStream(message) {
  return message.type === "stop_rtmp_stream" /* STOP_RTMP_STREAM */;
}
function isKeepRtmpStreamAlive(message) {
  return message.type === "keep_rtmp_stream_alive" /* KEEP_RTMP_STREAM_ALIVE */;
}
...
function isRtmpStreamRequest(message) {
  return message.type === "rtmp_stream_request" /* RTMP_STREAM_REQUEST */;
}
...
function isRtmpStreamStatus2(message) {
  return message.type === GlassesToCloudMessageType.RTMP_STREAM_STATUS;
}
...
var LEGACY_PERMISSION_MAP = new Map([
...
// src/constants/log-messages/logos.ts
var warnLog = String.raw`
__/\\\\____________/\\\\_        
 _\/\\\\\\________/\\\\\\_       
  _\/\\\//\\\____/\\\//\\\_      
   _\/\\\\///\\\/\\\/_\/\\\_     
...
var createPermissionWarning = (permissionName, funcName, packageName) => {
  ...
  const instructions = chalk.dim("Please enable this permission in the developer portal at:");
  const url = chalk.cyan.underline(`https://console.mentra.glass/apps/${packageName}/edit`);
...
var locationWarnLog = (cloudServerUrl, packageName, funcName) => {
  if (!cloudServerUrl)
    return;
  const permissionsUrl = `${cloudServerUrl}/api/public/permissions/${encodeURIComponent(packageName)}`;
  fetch(permissionsUrl).then(async (res) => {
...
var calendarWarnLog = (cloudServerUrl, packageName, funcName) => {
  if (!cloudServerUrl)
    return;
  const permissionsUrl = `${cloudServerUrl}/api/public/permissions/${encodeURIComponent(packageName)}`;
  fetch(permissionsUrl).then(async (res) => {
...
var readNotificationWarnLog = (cloudServerUrl, packageName, funcName) => {
  if (!cloudServerUrl)
    return;
  const permissionsUrl = `${cloudServerUrl}/api/public/permissions/${encodeURIComponent(packageName)}`;
  fetch(permissionsUrl).then(async (res) => {
...
var cameraWarnLog = (cloudServerUrl, packageName, funcName) => {
  if (!cloudServerUrl)
    return;
  const permissionsUrl = `${cloudServerUrl}/api/public/permissions/${encodeURIComponent(packageName)}`;
  fetch(permissionsUrl).then(async (res) => {
...
class SettingsManager {
  ...
  apiClient;
  mentraosSettings = {};
  mentraosEmitter = new EventEmitter2;
  subscribeFn;
  ...
  updateMentraosSettings(newSettings) {
    ...
    logger.debug({ newSettings }, `[SettingsManager] Updating MentraOS settings. New settings`);
    ...
    logger.debug({ mentraosSettings: this.mentraosSettings }, `[SettingsManager] Finished updating MentraOS settings. Current state:`);
  }
...
class CameraModule {
  ...
  async startStream(options) {
    ...
    const message = {
      type: "rtmp_stream_request" /* RTMP_STREAM_REQUEST */,
      packageName: this.packageName,
      sessionId: this.sessionId,
  ...
  async stopStream() {
    ...
    const message = {
      type: "rtmp_stream_stop" /* RTMP_STREAM_STOP */,
      packageName: this.packageName,
      sessionId: this.sessionId,
  ...
  subscribeToStreamStatusUpdates() {
    if (this.session) {
      this.session.subscribe("rtmp_stream_status" /* RTMP_STREAM_STATUS */);
    } else {
  ...
  unsubscribeFromStreamStatusUpdates() {
    if (this.session) {
      this.session.unsubscribe("rtmp_stream_status" /* RTMP_STREAM_STATUS */);
    }
  }
  onStreamStatus(handler) {
    ...
    return this.session.on("rtmp_stream_status" /* RTMP_STREAM_STATUS */, handler);
  }
...
class AppSession {
  ...
  constructor(config) {
    ...
    const { DashboardManager: DashboardManager2 } = (init_dashboard2(), __toCommonJS(exports_dashboard));
    this.dashboard = new DashboardManager2(this);
...
var createUpdateNotification = (versionNumb) => {
  const line = chalk2.bold.gray("-------------------------------");
  const logo = chalk2.cyan(mentraLogo_1);
  const title = chalk2.bold.cyan(newUpdateText);
  const versionMessage = `Version ${chalk2.bold.cyan(`SDK VERSION V${versionNumb} is out! \uD83C\uDF89`)}`;
  const currentNote = chalk2.yellow("You are running an older version");
  const instructions = chalk2.yellow("Update to the latest version with:");
  const command = chalk2.green.bold("bun install @mentra/sdk@latest");
  const content = [
    logo,
    title,
    line,
      ...
      this.app.listen(this.config.port, async () => {
        ...
        try {
          ...
          try {
            ...
            const response = await axios3.get(`https://${cloudHost}/api/sdk/version`, { timeout: 3000 });
...
export {
  ...
  isRtmpStreamStatus2 as isRtmpStreamStatusFromCloud,
  isRtmpStreamStatus,
  isRtmpStreamRequest,
  isRgbLedControlResponse as isRgbLedControlResponseFromGlasses,
  isRgbLedControlResponse2 as isRgbLedControlResponseFromCloud,
  ...
  isPhotoResponse2 as isPhotoResponseFromCloud,
  isPhotoResponse,
